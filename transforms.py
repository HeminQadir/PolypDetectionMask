...
# Flip the image either these ways:
# 0 for vertical, 1 for horizontal, -1 flipping around both axes. 
class RandomFlip:
    def __init__(self, prob=0.5):
        self.prob = prob
...
class Rotate:
    def __init__(self, limit1=0, limit2=360, prob=0.5):
        self.prob = prob
        self.limit1 = limit1
        self.limit2 = limit2
    ...
    def __call__(self, img, mask=None):
        if random.random() < self.prob:
            angle = random.uniform(self.limit1, self.limit2)
...
class Rescale:
    def __init__(self, limit1=0.35, limit2=0.75, prob=0.5):
        self.prob = prob
        self.limit1 = limit1
        self.limit2 = limit2
    ...
    def __call__(self, img, mask=None):
        if random.random() < self.prob:
            size = random.uniform(self.limit1, self.limit2)

            height, width = img.shape[0:2]
            mat = np.array([[size, 0.0, 0.0],
                             [0.0, size, 0.0]])
            img = cv2.warpAffine(img, mat, (height, width),
                                 flags=cv2.INTER_LINEAR,
                                 borderMode=cv2.BORDER_REFLECT_101)
...
class Zoomin:
    def __init__(self, limit1=0.01, limit2=0.05, prob=0.5):
        self.prob = prob
        self.limit1 = limit1
        self.limit2 = limit2
    ...
    def __call__(self, img, mask=None):
        if random.random() < self.prob:
            zoomin_rate = random.uniform(self.limit1, self.limit2)

            row, col = img.shape[0:2]

            row_factor = int(math.ceil(row*zoomin_rate))
            col_factor = int(math.ceil(col*zoomin_rate))
            
            img_zoom_org = img[col_factor:col_factor+(row-row_factor*2-1), row_factor:row_factor+(col-col_factor*2-1)]
            img = cv2.resize(img_zoom_org, ((col), (row)), interpolation=cv2.INTER_CUBIC)
            
            if mask is not None:
                img_zoom_gtc = mask[col_factor:col_factor+(row-row_factor*2-1), row_factor:row_factor+(col-col_factor*2-1)]
                mask = cv2.resize(img_zoom_gtc, ((col), (row)), interpolation=cv2.INTER_CUBIC)
...
# this is pre-processing for angiodysplasia-segmentation
class CenterCrop:
    def __init__(self, size):
...
# Hemin makes this class for CVC612 dataset 
class CropCVC612:
    ...
    def __call__(self, img, mask=None):
        ...
        img = img[16:h-10,44:w-30]

        if mask is not None:
            mask = mask[16:h-10,44:w-30]
...
class img_resize:
    ...
    def __call__(self, img, mask=None, size=512):
        h, w, c = img.shape
...
#pre_calacualted ImageNet mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)
class Normalize:
